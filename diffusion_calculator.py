...
class SignalFromCoefficients:
    def __init__(
        self,
        l_max: int,
        resp_coeff: torch.Tensor,
        cart_bvec: np.ndarray = None,
...
class SignalMultishell:
    ...
    def compute_signal_from_coeff(self, coeffs: torch.tensor):
        ...
        gm_signal = torch.einsum("bk, dk -> bd", gm_coeff, self.gm_coeff)
        csf_signal = torch.einsum("bk, dk -> bd", csf_coeff, self.csf_coeff)
