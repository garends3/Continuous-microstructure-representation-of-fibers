...
def parse_response(path: Path) -> np.ndarray:
...
def parse_bvals_col(path: Path) -> np.array:
    ...
    # return np.array([int(float(value)) for x.strip() in x], dtype=int)


def parse_c_bvecs_bvals_bdeltas(bvecs_path: Path, bvals_path: Path, bdeltas_path: Path) -> np.array:
    new_bvecs = np.load(bvecs_path)
    new_bvals = np.load(bvals_path)
    new_bdeltas = np.load(bdeltas_path)

    return new_bvecs, new_bvals, new_bdeltas


def save_bvecs(bvecs: np.ndarray, output_path: Path, type="row") -> None:
...
def flip_bvecs_col(bvec_path: Path, output_path: Path, axis: tuple[int, int, int]) -> None:
    bvecs = parse_bvecs_col(bvec_path)

    bvecs[:, 0] *= axis[0]
    bvecs[:, 1] *= axis[1]
    bvecs[:, 2] *= axis[2]

    save_bvecs(bvecs, output_path, type="col")
...
def spherical_to_cartesian(spherical_vec: np.ndarray) -> np.array:
    rho = spherical_vec[..., 0]
    phi = spherical_vec[..., 1]
...
def cartesian_to_spherical(cartesian_vec: np.ndarray) -> np.array:
    # (rho, phi, theta)
...
def sum_harmonics(
    coeffs: torch.tensor, lmax: int, thetas: np.ndarray, phis: np.ndarray
) -> torch.tensor:
    # MRtrix
...
def precompute_harmonics(
    lmax: int, thetas: np.ndarray, phis: np.ndarray
) -> torch.tensor:
...
def normalise(X_train, bvalues):
    nvol = len(X_train)
    normvol = np.where(bvalues == np.min(bvalues))[0]

    b0_mean = np.mean(X_train[:, normvol], 1)
    X_train = X_train / np.reshape(b0_mean, (len(b0_mean), 1))
